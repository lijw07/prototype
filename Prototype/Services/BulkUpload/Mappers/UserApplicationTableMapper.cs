...
public class UserApplicationTableMapper(
    IServiceScopeFactory scopeFactory,
    SentinelContext context,
    ILogger<UserApplicationTableMapper> logger)
    : ITableMapper
{
    ...
    public async Task<ValidationResultDto> ValidateRowAsync(DataRow row, int rowNumber, CancellationToken cancellationToken = default)
    {
        ...
        {
            logger.LogError(ex, "Error validating user application row {RowNumber}", rowNumber);
    ...
    public async Task<Result<bool>> SaveRowAsync(DataRow row, Guid userId, CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            cancellationToken.ThrowIfCancellationRequested();
            var defaultConnection = await context.ApplicationConnections
        ...
        {
            logger.LogError(ex, "Error saving user application row");
